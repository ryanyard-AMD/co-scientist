...
- **Optional Claude synthesis** (`--synthesize`): per method family, a Claude agent reads the retrieved chunks **and the extracted claims** and produces a grounded synthesis (narrative, key findings, reported metrics, hardware requirements, failure modes, open questions). Each failure mode is emitted as `{description, severity}` where the agent grades `severity` (`low`/`medium`/`high`) by how damaging the mode is to the goal's success criteria — severity is fixed at synthesis time (the only point an LLM reasons about the failure) and flows straight into the Score-stage risk penalty rather than being left null for a later pass. Claims are surfaced to the agent distinctly with their type, confidence, and relationship edges — `finding`/`contribution` claims feed key findings, `limitation` claims and `CONTRADICTS` edges feed failure modes, and `hypothesis` claims plus unresolved contradictions feed open questions. Pure RAG — the agent may cite only the `evidence_id`s it was given (claim records included), and any invented citation is stripped before persistence, so evidence-grounding audits stay valid. Off by default; requires `CS_ANTHROPIC_API_KEY`. If the LLM API rejects/fails during optional synthesis, Scout keeps the retrieved evidence and records a governance log instead of failing the whole scout run.
...
- **Corpus-derived, goal-scoped method taxonomy** (`cs ontology derive <goal_id>`): instead of forcing every goal into the fixed 7 seed method families, Claude induces the method families actually present in that goal's corpus from a broad retrieval sample when the LLM is available. If `CS_ANTHROPIC_API_KEY` is absent or the Anthropic API returns an error, derivation falls back deterministically to pinned families, GraphRAG method entity hints, runner-recognised anchor families mentioned in sampled chunks, and optional topic-cluster hints; the fallback is governance-logged and persists through the same goal-scoped `OntologyTerm` path. Terms with `workspace_id = NULL` are the shared global seed. Scout's term loader is goal-aware: per category, goal-scoped terms override the global seed when present (methods use the derived set; metric/hardware/failure_mode stay global). **`cs scout run` requires a derived method taxonomy for the goal and refuses (`422`) if none is persisted** — this replaces the old silent fall-back to the global seed, which could bucket a goal's evidence into the wrong domain (e.g. classifying sound-field-reproduction papers into the PSZ seed families). Use `--dry-run` to review induced families without persisting (note: a dry-run does **not** satisfy scout's precondition), and `cs ontology list -w <goal_id>` to inspect derived terms.
...
- Synthesis-backed generation: when a scout-stage `EvidenceSynthesis` exists for a method family, the card's mechanism summary, metrics, open questions, hardware, and risks come from the Claude synthesis; falls back to algorithmic extraction from evidence groups otherwise. The card trusts the synthesis's grounded `hardware_requirements` and only unions the noisier per-record keyword `hardware_assumptions` when the synthesis surfaced no hardware at all (prevents generic hardware — e.g. "headphone", "soundbar" — leaking into a card whose synthesis named specific rig hardware). Each risk carries the synthesis-assigned `severity`, so the Score-stage risk penalty engages at generation without waiting for a revise pass.
...
- **Physics delegated to the Experimentation System.** The co-scientist holds no numpy; it resolves the geometry and calls repro's `POST /api/v1/device-sim`, which runs the same validated ACC physics repro's paper reproductions use — image-source room ATFs plus a per-element **Berktay directivity** model of the PAL's nonlinear ultrasonic demodulation, with a realistic ~−25 dB off-axis floor and a near-field beam-broadening taper — and returns broadband + per-band contrast. Gated by the execution boundary (`assert_execution_boundary`) like every other repro-compute path.
- **Sound-field reproduction quality** with `cs device reproduce <device_id> <goal_id> --target spherical_wave` calls repro's `POST /api/v1/device-sim/reproduce`, solves a pressure-matching field reproduction problem over a listener-region control grid, and persists full reproduction metrics: normalized reproduction error, spatial correlation, mean/max SPL error, array effort, per-band values, plus acoustic contrast as a secondary metric. Use this for OPL/PM concepts where the question is "how closely does the device reproduce the target field?", not just "how much contrast can it create?"
- **Deterministic geometry resolution** (`_resolve_geometry`) is a four-layer merge, later layers winning: (1) simulator defaults → (2) prose inference, reading element count and layout from `hardware.speakers` and boresight distance from `form_factor.listener_distance_cm` → (3) the card's `geometry` block → (4) per-run `--set` overrides. The same card always resolves to the same geometry, so re-runs are reproducible, and the resolved knobs are echoed back for transparency. Layer 2 exists for cards written before the `geometry` column; a card with `geometry = {}` resolves identically to how it always did. When a block moves the `listener` without saying where the dark zone went, the dark zone follows it at the same 40 cm lateral offset rather than staying pinned at the old boresight distance.
- **Distributed ring geometry**: device cards whose speaker geometry mentions a ring, periphery, azimuthal placement, distributed modules, or surround layout resolve to `layout=ring`; `ring_radius` is a first-class override/sweep knob for tabletop multi-PAL arrays.
- **Geometry block on the card** in a `geometry` JSON column (Alembic `0034`): the sim-ready knobs the card itself commits to, in the same names repro's `DeviceGeometryRequest` uses (metres, boresight `+y`). `{}` on legacy cards, which then resolve exactly as they did before the column existed. `positions`/`normals` are deliberately not part of this block — a card picks a `layout`; explicit element coordinates stay an override-only escape hatch.
...
- **Optimize across many** with `cs device optimize <device_id> <goal_id> --sweep n_elements=8,12,16 --sweep sidelobe_floor=0.056,0.02` — the co-scientist forwards the sweep to repro's `POST /api/v1/device-sim/optimize`, which simulates the cartesian product, ranks by contrast, and returns the best plus every candidate. The reverberant room is cached on the resolved element positions, so weight-only sweeps cost a single room build. The winning geometry is written onto the card exactly like `simulate` (so the roadmap picks it up), and the CLI prints the ranked table and the gain over the previous prediction. A numeric knob whose winner lands on the edge of its swept range is reported as a **boundary optimum** — the sweep never bracketed a maximum, so the value records where the range stopped rather than where the design is best (contrast is monotone in `ring_radius`, for instance; see [docs/device-geometry.md](docs/device-geometry.md)). Add `--roadmap` to regenerate the goal's research roadmap from the refined card in the same command, closing the loop from an optimized geometry to updated next-best actions.
...
The stripped metric name is then **canonicalized** onto the `METRIC_NAMES` vocabulary via
`domain.canonicalize_metric()` (e.g. `acoustic_contrast_min` → metric `acoustic_contrast_db`), so a
pass condition reconciles with the canonical name the reproduction emits — otherwise a metric the
run *did* measure would be falsely flagged `unmeasurable`. The chosen `experiment_id` is added to
the proposal before design-run.
