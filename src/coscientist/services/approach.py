...
def _run_revise_agent(
    ...
    system_prompt = (
        ...
        "labelled as a non-target-device analogue, since leaving it in the metrics table lets "
        "downstream stages misread it as achievable on THIS device. When the critic asks you to "
        "quantify a device limit it named (e.g. the spherical mic array's spatial-aliasing "
        "frequency or usable spherical-harmonic order for its radius across the target band), "
        "compute and state the figure. Use device_relevance for the narrative, the structured "
        "lists for the itemised content. Record the revision by calling the record_revision tool. "
        "Cite ONLY "
